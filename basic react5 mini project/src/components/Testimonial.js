...
export default function Testimonial(props){
    ...
    function surpriseHandler(){
        let randomIndex = Math.floor(Math.random() * props.reviews.length);
    ...
    return (
        <div className="w-[85vw] md:w-[700px] bg-white flex flex-col justify-center items-center mt-10 p-10 transition-all duration-700 hover:shadow-xl rounded-md">
            <Card review={props.reviews[index]} />
            ...
            <div className='mt-5'>
                <button onClick={surpriseHandler} className='bg-violet-500 hover:bg-violet-600 transition-all duration-200 px-10 py-2 rounded-md font-bold text-white text-lg cursor-pointer'>Surprise Me</button>
            </div>
        </div>
    )
}